class CardWordSubPart {
  word: string;
  constructor(wrd: string) { this.word = wrd; }
}
...
class CardWord {
  ...
  constructor(contents: string) {
    ...
    contents.split(' ').forEach(piece => this.subparts.push(new CardWordSubPart(piece)));
  }
}

function appendArrayNTimes<T>(arr: T[], n: number): T[] {
  if (!Number.isInteger(n) || n <= 0) return arr;
...
export class CardBufferManager {
  ...
  getNextMorse(repeats = 0, additionalWordSpaces = 0): string {
    if (!this.hasMoreMorse()) this.populateBuffer(repeats, additionalWordSpaces);
